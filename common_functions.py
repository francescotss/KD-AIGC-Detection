...
def Make_DataLoader(dir,
    ...
    dic_CoReD = None
    if TRAIN_MODE:
        if name_mixed_folder : MIXED_MODE = True
        else: MIXED_MODE = False
        val_target_dir_mixed=''
        val_target_loader_mixed=None
        train_dir = os.path.join(dir,name_mixed_folder if MIXED_MODE else name_target,'train')
        #For Validataion
        ...
        print("Validation Source dir", val_source_dir)
        print("Validation Target dir", val_target_dir)
...
def Make_DataLoader_continual(dir,
    ...
    train_dir = os.path.join(dir, '{}/train/'.format(name_target if not name_mixed_folder else name_mixed_folder))
    #For Validataion
    val_target_loader_mixed=None
    val_target_dir_MIXED = ''
    source_dataset=[]
...
def Test_PRF(test_loader, model, criterion, log=None): # precision/recall/f1-score
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets) in tqdm.tqdm(enumerate(test_loader)):
            inputs, targets = inputs.cuda(),torch.from_numpy(np.array(targets)).cuda()
...
def get_augs(args=None):
    if args is not None and args.crop:
        resize_func = transforms.RandomCrop(args.resolution, pad_if_needed=True)
    else:
        resize_func = transforms.Resize((args.resolution, args.resolution))
        
    if args is not None and args.flip:
...
def load_models(weight, nameNet='ResNet', num_gpu='', TrainMode=True):
    ...
    if weight != "":
        if os.path.isdir(weight):
            print(f"Loading {nameNet} from {weight}/model_best_accuracy.pth")
            checkpoint =torch.load(weight+'/model_best_accuracy.pth', map_location=device)
    ...
    #model load
    if nameNet=='Xception':
        teacher_model = xception(num_classes=2, pretrained='')
        student_model = xception(num_classes=2, pretrained='')

    elif nameNet=='ResNet':
        teacher_model = resnet50(pretrained=pretrained, num_classes=1)
        student_model = resnet50(pretrained=pretrained, num_classes=1)
    elif nameNet=='ResNet18':
        teacher_model = resnet18(pretrained=pretrained, num_classes=1)
        student_model = resnet18(pretrained=pretrained, num_classes=1)
    elif nameNet=='MobileNet2':
        teacher_model = mobilenet_v2(pretrained=pretrained, num_classes=2)
        student_model = mobilenet_v2(pretrained=pretrained, num_classes=2)
...
def save_checkpoint(state, checkpoint, filename='checkpoint.pth' , AUC_BEST = False, ACC_BEST = False):
    name_save= filename if filename else ''
    if AUC_BEST : name_save = 'model_best_auc'
    if ACC_BEST : name_save = 'model_best_accuracy'

    filepath = os.path.join(checkpoint, name_save+'.pth')
